...
/* ─────────────────────────────────────────────────────────────────────────
   Scroll-based velocity marquee (Magic UI's scroll-based-velocity, adapted).

   Rows loop continuously and speed up with the page's scroll velocity,
   reversing direction when you scroll back up. Adapted for this codebase:
   imports come from framer-motion (already a dependency) rather than the
   standalone `motion` package, and cn from @/lib/utils/cn.

   Cheap when idle: the animation frame bails while the row is offscreen or
   the tab is hidden, and reduced-motion pins the speed multiplier to 1 so
   the text still loops but never accelerates.
   ──────────────────────────────────────────────────────────────────────── */
...
/** Shares one velocity spring across every row inside it. */
  ...
  useEffect(() => {
    ...
    if (container && block) {
      const updateSizes = () => {
        ...
        unitWidth.set(bw);
        // Enough copies to cover the container plus a spare either side, so
        // the seam never enters the viewport mid-loop.
...
/** Fallback when a row is used outside a container: its own velocity spring. */
